...
class SolverWrapper(object):
    ...
    def get_curr_roidb(self):
        if self.solver.iter >=self.start_iters: 
            ...
            self.object_num += self._step_num
    ...
    def Roidb_detect_and_sort(self):
        ...
        for i in xrange(n_imgs):   
            ...
            #     detected_labels = np.hstack((detected_labels, cls_j))
...
#----------------------------------begin yanxp 10.15-----------------------
            ...
            for cls_id in xrange(labels_length):
                ...
                for j in xrange(per_class_proposals_num):
                    ...
                    cv2.imwrite('comb/'+image_name,im)
                    # cv2.imshow('image',im)
                    # cv2.waitKey(0)
                    # cv2.destroyAllWindows()
                    # print "original class:",CLASSES[cls_list[index_num]]
                    curre_select_num = 0
                    total_select_num = 3
                    # print "start:"
                    for k in xrange(n_imgs):
                        ...
                        seletc_img_labels= roidb[select_id]['img_labels']
                        select_boxes = roidb[select_id]['boxes']
                        
                        if curr_shape[0]>seletc_im_shape[0] or curr_shape[1]>seletc_im_shape[1] or select_id == i or curr_img_labels in seletc_img_labels :
                            continue
                        curre_select_num += 1
                        ...
                        if proposal_width_original*proposal_height_original >0.6*(seletc_im_shape[0]*seletc_im_shape[1]):
                        ...
                        if seletc_im.shape[0]<=proposal_height or seletc_im_shape[1]<=proposal_width:
                            continue
                        ...
                        # cv2.rectangle(combination_im,(int(latent_box[0]),int(latent_box[1])),
                        #                  (int(latent_box[2]),int(latent_box[3])),(255,0,0),2)

                        # cv2.rectangle(combination_im,(int(pred_latent_boxes[0]),int(pred_latent_boxes[1])),
                        #                  (int(pred_latent_boxes[2]),int(pred_latent_boxes[3])),(0,0,255),2)

                        # jpg_path = 'comb/'+image_name
                        # if not os.path.exists(jpg_path):
                        #     os.makedirs(jpg_path)

                        # cv2.imwrite(jpg_path+'/'+str(select_id)+'.jpg',combination_im)
                        # if curre_select_num ==1 :
                        #     cv2.imwrite('comb/'+image_name+'.jpg',combination_im)
